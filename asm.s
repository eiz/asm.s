...
// ── ELF constants ─────────────────────────────────────────────────────────
.equ CODE_START,      112                   // 64-byte ehdr + 56-byte phdr, overlapping by 8
...
.equ TIER_TBL_SIZE,      36                 // (18 tiers + raw escape) * 2 bytes
.equ DICT_SIZE,         578                 // table + packed dicts (exact, unpadded)
.equ COND_DICT_OFF,     549                 // ('b',0) dict offset within the blob
.equ RAW_CODE,          204                 // 1 + total dictionary codes

// ── section IDs ───────────────────────────────────────────────────────────
.equ SYMF_SEC_SHIFT, 3
.equ SEC_TEXT,       0                      // pre-multiplied by 8 for direct state block indexing
.equ SEC_RODATA,     1 << SYMF_SEC_SHIFT
.equ SEC_BSS,        2 << SYMF_SEC_SHIFT

// ── state block offsets (all u64) ─────────────────────────────────────────
.equ ST_TEXT_POS,    0                      // current offset within .text
.equ ST_RODATA_POS,  ST_TEXT_POS + 8        // current offset within .rodata
.equ ST_BSS_POS,     ST_RODATA_POS + 8      // current offset within .bss
.equ ST_CUR_SEC,     24                     // current section (SEC_TEXT/RODATA/BSS)
.equ ST_TEXT_BASE,   32                     // virtual address of .text start
.equ ST_RODATA_BASE, ST_TEXT_BASE + 8       // virtual address of .rodata start
.equ ST_BSS_BASE,    ST_RODATA_BASE + 8     // virtual address of .bss start
...
.equ SYM_NAME_PTR,   0
.equ SYM_NAME_LEN,   SYM_NAME_PTR + 8
.equ SYM_FLAGS,      SYM_NAME_LEN + 8
.equ SYM_VALUE,      SYM_FLAGS + 8
.equ SYM_ENT_SIZE,   SYM_VALUE + 8
...
.equ SYM_TBL_BYTES,   SYM_TBL_SLOTS * SYM_ENT_SIZE
.equ RELOC_MAX_ENTRIES, 32768
.equ RELOC_TBL_BYTES,   RELOC_MAX_ENTRIES * 8
...
state:        .skip ST_SIZE
              .skip INPUT_BUF_SIZE          // input_buf
              .skip TEXT_BUF_SIZE           // text_buf (x27)
              .skip RODATA_BUF_SIZE         // rodata_buf (x27 + x29)
...
msg_create:
    ...
    .byte   0xf3, 0xd1, 0xb9, 0x0b, 0x5a, 0x84, 0x03
msg_badimm:
...
_start:
    ...
    // pin x27 = text_buf (x28 + 1 MB input buffer + state header)
    orr     x27, xzr, #1048576
    add     x27, x28, x27
    add     x27, x27, #INPUT_BUF_OFF
    // pin x29 = parse_x23_ws: first-register-operand parses call blr x29,
    // so every such call site is the same word. The pass tree preserves x29
    // (it previously held the 1 MB stride constant, now materialized inline).
    adr     x29, parse_x23_ws
    // pin x7 = parse_operand: every register/immediate operand parse calls
    // through blr x7, so all those call words are identical. The pass tree is
    // audited to preserve x7 (it previously carried the numeric-label table).
    adr     x7, parse_operand
    ...
    csel    x6, x6, x9, ne                  // pin appended stub (stage 0) or inherited stub
    ...
    mov     w16, #0                         // pinned text base: zero in pass 1
    bl      run_pass

    // ── compute section base addresses ────────────────────────────────────
    ldp     x18, x21, [x28, #ST_TEXT_POS]   // text_pos, rodata_pos (survive pass 2)
    ...
    // stub; the padded in-memory span is used only to place bss. x18 retains
    ...
    ldr     x14, [x28, #ST_ALIGN_MASK]
    orr     x14, x14, #15                   //      = 2^max(align,4) - 1
    add     x18, x18, x14
    bic     x18, x18, x14
    add     x9, x21, x14                    // keep x21 = raw rodata bytes for the file
    bic     x9, x9, x14                     // x9 = padded in-memory rodata span
    mov     w16, #CODE_START                // pinned text base: nonzero in pass 2
    add     x1, x18, #CODE_START            // rodata_base = text_base + text size
    stp     x16, x1, [x28, #ST_TEXT_BASE]
    ...
    add     x20, x2, x9                     // mem_size = bss_base + bss_size
                                            // (x18-x21 survive run_pass; x14-x26 do NOT)
...
ct_located:
    ...
    // x10 = 0, its rotation is 0 (w8 = the verbatim word), and x9 = 4, so
    // the generic hit path emits it with no dedicated setup.
    cbz     x10, ct_hit
    lsl     w0, w9, #3                      // s = 8 * nlit
...
ct_done:
    ...
    // The header, stub, and dictionaries are contiguous in both layouts,
    ...
    orr     x9, xzr, #1048576
    add     x9, x27, x9                     // rodata_buf
    add     x10, x21, #0                    // raw rodata bytes
    bl      copy_bytes
    adr     x9, reloc_table
    sub     x10, x19, x9                    // relocation bytes
...
parse_ident:
    orr     x2, xzr, x0                     // working pointer (x0 = start, preserved)
    ldrb    w9, [x2]
    b       pi_check_first
1:  ldrb    w9, [x2, #1]!
...
pi_check_first:
    ...
    ret
pe_ops:
    orr     x18, x18, x0                    // opcode 0: |
    ret
    and     x18, x18, x0                    // opcode 1: &
    ret
    add     x18, x18, x0                    // opcode 2: +
    ret
    sub     x18, x18, x0                    // opcode 3: -
    ret
    mul     x18, x18, x0                    // opcode 4: *
    ret
    lsl     x18, x18, x0                    // opcode 5: <<
    ret
    lsr     x18, x18, x0                    // opcode 6: >>
    ret

// terminate_line — advance x18 past the next NUL or newline, replacing a
...
open_ro:
    ...
    tbnz    x0, #63, err_open
    add     x1, x28, #INPUT_BUF_OFF
    orr     x2, xzr, #1048576               // read cap = 1 MB
    mov     w8, #SYS_read
    ret
...
run_pass:
    ...
    // Every callee in the pass tree is audited to preserve x4/x5/x7.
...
1:  sub     x25, x25, #(NUMLAB_MAX_DEFS * 4)
    ...
    add     x18, x28, #INPUT_BUF_OFF        // input_buf
    ldr     x10, [x28, #ST_MAIN_LEN]
    str     x10, [x28, #ST_INPUT_LEN]
    add     x20, x18, x10                   // x20 = end of current file
...
pl_check_content:
    sub     x18, x0, #0
    // empty line? (w9 pre-loaded by skip_ws / ws_x18)
    cbz     w9, pl_done
    ...
    adr     x11, numlab_defs
    add     x11, x11, x10, lsl #8           // numlab_defs + digit * 64 * 4
...
pl_directive:
    // x0 still equals x18 here (set at pl_check_content, untouched since)
    add     x0, x0, #1                      // skip '.'
    bl      parse_ident
    // x0 = name start, x1 = name length, x2 = end pointer
    add     x18, x2, #0                     // position after directive name
...
// append/read the child, and tail-enter the shared source traversal loop.
    add     x18, x18, #2                    // skip the single space and opening quote
    add     x1, x18, #0                     // path start for open + diagnostics
1:  ldrb    w11, [x18], #1                  // find closing quote (pass 2 sees its NUL)
    cmp     w11, #'"'
    b.eq    2f
    cbnz    w11, 1b
2:  strb    wzr, [x18, #-1]
    ...
    add     x18, x1, #0                     // cursor = start of included content
    add     x20, x1, x0                     // end of included content
    add     x9, x9, x0                      // grow the content length, bounded by
    tbnz    x9, #20, err_overflow           // bit 20 crosses the 1 MB input bound
    str     x9, [x28, #ST_INPUT_LEN]
    sub     x0, x25, #0                     // close fd before descending
    mov     w8, #SYS_close
    svc     #0
    mov     w5, #0
    add     x4, x4, #1
...
// .ascii/.asciz "string" — w10 still holds directive[4] ('i' or 'z')
    ubfx    w11, w10, #1, #1                // suffix 'i'/'z' bit 1 = null flag
    bl      ws_x18                          // x0 = pointer to '"'
...
ps_done:
    ...
    add     x0, x11, x0                     // not counted, overwritten by next emit)
advance_sec_pos:
    ldr     x10, [x28, x26]
    add     x10, x10, x0
    str     x10, [x28, x26]
...
dir_word:
    ...
    mov     w14, #4
dir_data_common:
    // .byte/.hword/.word/.dword <expr>[, <expr>…] — emit each at width x14
    // (x14 + x27 preserved across parse_expr)
1:  bl      parse_expr0_x19
    ...
    add     x18, x2, #1                     // past the comma → next value
    b       1b
...
// strlen_write2 — write the null-terminated string in x1 to stderr;
// computes the length, then falls through into write2.
strlen_write2:
    add     x2, x1, #0
1:  ldrb    w9, [x2], #1
    cbnz    w9, 1b
...
1:  ldrb    w9, [x0, #1]!
    ...
    add     w10, w11, w10                   // lsl=0, lsr=1, asr=2, ror=3
...
// ──────────────────────────────────────────────────────────────────────────
//  parse_operand — blr x7 target: route on the preloaded first character.
//  Register names start at or above 'a' (w/x/s); '#' and ':' sit below it,
//  so one compare selects parse_register or the fall-through immediate parse.
//  skip_lsl falls in from above with w9 = '#', which routes identically.
// ──────────────────────────────────────────────────────────────────────────
parse_operand:
    cmp     w9, #'a'
    b.hs    parse_register
...
//  returns x14 = Rd, x23 = sf, x0 = Rn
//  NOTE: uses [sp, #56] for return address; called from the instruction encoder
// ──────────────────────────────────────────────────────────────────────────
parse_2reg:
    str     x30, [sp, #56]
    bl      parse_x14_ws
...
2:  ldrb    w9, [x2, #1]!
    ...
    b.hi    parse_int_ret
    add     w10, w10, #10
4:  madd    x0, x8, x0, x10                 // acc = acc*radix + digit
    b       2b

// ──────────────────────────────────────────────────────────────────────────
//  parse_label_pc_rel — parse a label reference and compute its PC-relative
//  offset in instruction units; x22 preserves the caller across the parser
// ──────────────────────────────────────────────────────────────────────────
parse_label_pc_rel:
    sub     x22, x30, #0
    bl      parse_label_ref
...
parse_label_ref:
    ...
    add     x0, x9, x0                      // 1-indexed: fwd = cursor+1, back = cursor
    ldr     w0, [x11, x0, lsl #2]
    // defs are stored as .text offsets; add the text base (refs are pass-2 only)
    mov     w9, #SEC_TEXT                   // section = .text for pea_sec_base
    b       pea_sec_base
...
pe_atom_ident:
    ...
    bl      sym_lookup
    cbnz    x12, 1f
    cbnz    x16, err_undef                  // pass 1 reads the zeroed empty entry
...
//  returns x25 = Rn, x18 = imm (or Rm), x26 = mode:
//    0 = base / signed offset, 1 = post-index, 3 = pre-index,
//    option<<4 | S<<3 | 4 = register offset (x18 = Rm)
//  clobbers x18
// ──────────────────────────────────────────────────────────────────────────
parse_mem:
    str     x30, [sp, #56]
    bl      skip1_ws                        // skip '['
    blr     x7                              // parse_register: Rn
...
2:  ldrb    w9, [x0, #1]!
    ...
    cbz     w9, pe_atom_err
    b       2b
3:  blr     x7                              // parse_hash_imm: x0=shift; x26 preserved
    cbz     x0, 4f
...
// parse_x14_ws — parse the first register into x14, save sf in x23, then skip
// the comma and whitespace. Shares the x22 LR-save convention with other helpers.
parse_x14_ws:
    sub     x22, x30, #0
    blr     x29                             // parse_x23_ws
    add     x14, x23, #0
    sub     x23, x24, #0                    // sf
    br      x22
...
//  returns x14 = Rd, x23 = sf, x24 = Rn, x0 = Rm
//  NOTE: uses [sp, #56] for return address; called from the instruction encoder
// ──────────────────────────────────────────────────────────────────────────
parse_3reg:
    str     x30, [sp, #56]
...
1:  sub     w1, w9, #'w'                    // 'w' → 0, 'x' → 1 (sf)
    ...
    b.hi    parse_reg_fail
    ldrb    w9, [x0, #1]
    cmp     w9, #'z'                        // (same words as parse_zn_reg's check)
    b.ne    parse_reg_num
    add     x2, x0, #3
    b       2b
...
parse_reg_num:
    ...
    sub     w10, w9, #'0'                   // first digit (canonical digit check)
    cmp     w10, #9
    b.hi    parse_reg_fail
    ldrb    w9, [x0, #2]
    sub     w11, w9, #'0'
    ...
    add     w13, w10, w10, lsl #2           // first * 5
    add     w10, w11, w13, lsl #1           // second + first * 10
    add     x2, x2, #1
1:  cmp     w10, #30
...
// ──────────────────────────────────────────────────────────────────────────
//  pl_instruction — dispatch a mnemonic, parse its operands, and emit
...
pl_instruction:
    ...
    // x18 already equals x0 (set by process_line before parse_ident)
    add     x20, x1, #0                     // no-operand forms (ret/nop) never parse
    add     x21, x2, #0
    // Every returning encoder helper preserves x17, so pin the common emit
    // continuation and make all direct emit branches share one instruction word.
    adr     x17, emit_inst_done
...
ei_rbit:
    ...
    bl      parse_2reg                      // x14=Rd, x23=sf, x0=Rn
    orr     w0, w14, w0, lsl #5             // Rd | (Rn << 5)
...
// Reached by branch from the instruction encoder; synthesize emit_word_raw's return
...
ei_dispatch:
    ...
    // free alignment byte (never indexed): value chosen for the dictionary
    .byte   0x91
    .align  2
ei_n_entry:
    cmp     w10, #'o'
...
ei_nop:
    ldrb    w10, [x18, #2]
    add     w10, w10, w10, lsr #2           // p/e/i select NOP/WFE/WFI field 0/2/3
    movz    w0, #0x201F
    bfi     w0, w10, #5, #2
ei_sysfixed_tail:
...
ei_d:
    ...
    bl      ws_x21
    blr     x7                              // parse_hash_imm: x0 = barrier CRm
...
ei_a:
    ...
    sbfx    w14, w20, #2, #1                // AND=0; ANDS=-1 (low opc bits = 3)
    b       ei_logical
ei_o_entry:
    mov     w14, #1                         // ORR opc
ei_logical:
...
pm_close:
    ...
    b.ne    pm_store_done
    bl      skip1_ws                        // skip ','
    blr     x7                              // parse_hash_imm; parse_expr stores cursor
    sub     x18, x0, #0
...
// lsl/lsr — immediate (UBFM alias) or register (LSLV/LSRV)
    ...
    orr     w25, w25, #0x2000
ei_shift_common:
    bl      parse_2reg                      // x14=Rd, x23=sf, x0=Rn, x2=ptr past
    bl      mov_x24_skip                    // Rn → x24, skip ','
    cmp     w9, #'#'
    b.eq    ei_shift_imm_dispatch
    // register form
    blr     x7                              // parse_register: Rm
    b       emit_3reg_1AC0_tail
ei_isb:
    movz    w0, #0x3FDF
    b       ei_sysfixed_tail

// parse_zn_reg — validate a z/n suffix, set x14=0/1, then parse first reg
parse_zn_reg:
    ldrb    w10, [x18, #2]
    cmp     w10, #'z'
    cset    x14, ne
...
ei_t_entry:
    ...
    bl      parse_zn_reg                    // x14=0 for tbz, 1 for tbnz
    blr     x7                              // parse_hash_imm
...
ei_h:
ei_svc:
    ubfx    w8, w9, #3, #1                  // 'h' bit 3 = 1, 's' bit 3 = 0
    add     w25, w8, #1                     // exception field: hvc=2, svc=1
    bl      ws_x21
    blr     x7                              // parse_hash_imm: x0 = imm16
...
ei_bfm_unified:
    bl      parse_2reg                      // x14=Rd, x23=sf, x0=Rn, x2=ptr past
    bl      mov_x24_skip                    // Rn → x24, skip ','
    blr     x7                              // parse_hash_imm: #op3
    add     x25, x0, #0
    bl      ws_x2_skip1                     // skip ','
    blr     x7                              // parse_hash_imm: #op4
    add     x11, x0, #0                     // op4 in x11
    // determine base opcode from mnemonic first char (x18 preserved)
    ldrb    w9, [x18]
    ubfx    w13, w9, #4, #2                 // suffix offset: b*=2, u*/s*=3
    eor     w9, w9, #3                      // b/s/u low bits become opc 1/0/2
    movz    w0, #0x1300, lsl #16            // SBFM base
    bfi     w0, w9, #29, #2
    ldrb    w13, [x18, x13]                 // load distinguishing char
...
// insert: immr=(-lsb) mod size, imms=width-1 (fall-through from dispatch)
    mov     x9, #31
    add     x9, x9, x23, lsl #5             // size-1 (same words as shift-immediate path)
    neg     x10, x25
    and     x10, x9, x10
...
ei_csel_common:
    ...
    sub     x0, x25, #0                     // Rm for emit_3reg_sf_tail
ei_csel_tail:
...
ei_tst:
    mov     w14, #3                         // ANDS opc
    adr     x22, ei_logical_operand
    b       ei_rn_xzr_rd

// cmp/cmn Rn, #imm / cmp/cmn Rn, Rm — reuse addsub with Rd=xzr
ei_c_cm:
    ldrb    w10, [x18, #2]
    eor     w14, w10, #3                    // 'p' low bits 0→3, 'n' 2→1
    adr     x22, ei_addsub_operand
// Shared by tst/cmp/cmn: parse Rn, force Rd=XZR, continue via x22.
ei_rn_xzr_rd:
    blr     x29                             // parse_x23_ws: x23=Rn, x24=sf
    add     x25, x23, #0                    // Rn
    mov     x23, #31                        // Rd = xzr
    br      x22
...
ei_br:
ei_blr:
    // parse_x23_ws parses the sole register; its trailing comma-skip halts
    // harmlessly at the line's NUL, and x24/x15 are dead here.
    blr     x29                             // parse_x23_ws
...
ei_shift_imm_dispatch:
    ...
    // size-1 in x9 with the same words the bitfield-insert path uses
    mov     x9, #31
    add     x9, x9, x23, lsl #5             // size-1 = 31 or 63 (shared)
    tst     w25, #0xC00
    b.ne    ei_lsr_asr_imm
    // LSL #n: UBFM Rd, Rn, #(-n mod size), #(size-1-n)
    neg     x10, x0
    and     x10, x9, x10                    // immr = (-n) & (size-1)
    sub     x11, x9, x0                     // imms = (size-1) - n
    b       ei_ubfm_emit

ei_lsr_asr_imm:
    add     x10, x0, #0                     // immr = n
    add     x11, x9, #0                     // imms = size-1
...
ei_ld:
ei_st:
    ubfx    x14, x9, #2, #1                 // 'l' bit 2 = 1, 's' bit 2 = 0
    ldrb    w10, [x18, #2]
    cmp     w10, #'p'
    b.eq    ei_ldst_pair
    blr     x29                             // parse_x23_ws: x23=Rt, x24=sf
    // x24 already has sf from parse_x23_ws
    cmp     w20, #3                         // len 3 = plain ldr/str; else suffixed
    // precompute size encoding: 0=byte, 1=half, 2=32bit, 3=64bit
    add     w20, w24, #2                    // 2 or 3 (leaves the flags alone)
    b.eq    1f                              // len=3: use sf+2
    ldrb    w10, [x18, #3]                  // 'b'=0x62, 'h'=0x68, 's'=0x73
    tbnz    w10, #0, ei_ldrs_size           // sign-extending load (ldrsb/ldrsh/ldrsw)
    ubfx    w20, w10, #3, #2                // 0 for byte, 1 for half
    b       ei_ldst_bracket                 // suffixed forms always take a bracket
    // Literal-load check: ldr Rt, label has no opening bracket.
1:
    cbz     x14, ei_ldst_bracket            // store: must have [
    cmp     w9, #'='
    b.eq    ei_ldr_eq                       // ldr Rt, =label  (pseudo-op)
    cmp     w9, #'['
    b.ne    ei_ldr_literal
ei_ldst_bracket:
    bl      parse_mem                       // x25=Rn, x18=imm/Rm, x26=mode
...
ei_ldst_uimm_encode:
    tbnz    x18, #63, ei_ldst_unscaled      // negative → LDUR/STUR encoding
    lsr     x10, x18, x20
    lsl     x9, x10, x20
    cmp     x9, x18
...
ei_addsub_operand:
    ...
    // register form: Rd, Rn, Rm [, lsl #N] — w26=base, x25=Rn, x14=op bits
ei_reg_operand:
    blr     x7                              // parse_register
...
ei_ldst_reg:
    orr     x24, x26, x18, lsl #7           // prepack mode | (Rm << 7)
    bl      ldst_base
...
// reads x20=size, x14=opc, w23=Rt, x25=Rn; returns w0=partial insn
ldst_base:
    orr     w0, w23, w25, lsl #5
    orr     w0, w0, w20, lsl #30
    orr     w0, w0, w14, lsl #22
    ret
...
ei_ldrs_size:
    ldrb    w9, [x18, #4]                   // 5th char: 'b','h','w'
    ubfx    w20, w9, #3, #2                 // 'b'→0, 'h'→1, 'w'→2
    eor     w14, w24, #3                    // opc = 3 - sf (Xd→2, Wd→3) = sf^3
    b       ei_ldst_bracket
...
ei_ldst_pair:
    // x14=L (already set by ei_ld/ei_st)
    blr     x29                             // parse_x23_ws
    add     w21, w24, #2                    // offset scale: 2 (32-bit) or 3 (64-bit)
    blr     x7                              // parse_register: Rt2
    bl      mov_x24_skip                    // Rt2 → x24, skip ',' → at '['
    bl      parse_mem                       // x25=Rn, x18=imm, x26=mode
...
eli_start:
    ...
    // val in x0 is dead once validated, so pack the result in place
    sub     x0, x22, x13, lsl #1
    sub     x0, x0, #1
    and     x0, x0, #0x3F                   // imms
...
1:  eor     w26, w26, #0x40000000           // MOVZ base → MOVN base
    b       ei_mov_try_phase
// Dead pad word: the compressor covers .text padded to 16, and an explicit
// dictionary-cheap word costs less in the stream than an implicit zero word.
    add     x0, x0, #1
